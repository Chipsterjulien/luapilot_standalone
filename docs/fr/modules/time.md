...
`luapilot.time` expose `CLOCK_MONOTONIC` (durées) et
`CLOCK_REALTIME` (timestamps), avec `sleep()` qui respecte
l'interruption par signal.

## API

| Fonction | Renvoie |
| --- | --- |
| `luapilot.time.monotonic()` | `number` — secondes depuis une epoch arbitraire (immune aux changements d'horloge) |
| `luapilot.time.realtime()` | `number` — temps POSIX (secondes depuis 1970-01-01 UTC) |
...
- **`monotonic()` / `realtime()`** : ne peuvent pas échouer.
  Renvoient toujours un number.
- **`sleep(amount, unit)`** :
...
- **Deux horloges, deux fonctions, pas de flag**. `monotonic()`
  pour les durées, `realtime()` pour les timestamps. Les fondre
  en une fonction avec paramètre inviterait au mauvais choix.
...
- **Pas de helper "format date"**. `os.date(format, realtime())`
  marche très bien, et une fonction dupliquée ne serait qu'un
  wrapper.

## Hors v1

- `luapilot.time.boottime()` (`CLOCK_BOOTTIME` — inclut le
  suspend). Rarement utile, facile à ajouter plus tard.
- Helpers `luapilot.time.utcnow()` / `localnow()` qui renvoient
  des strings pré-formatées. Trivial en Lua, sans valeur ajoutée.
