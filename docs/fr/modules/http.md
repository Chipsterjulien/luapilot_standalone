...
| `babet.http.put(url, opts?)` | raccourci pour `request{ method="PUT", url=url, ... }` |
| `babet.http.delete(url, opts?)` | raccourci pour `request{ method="DELETE", url=url, ... }` |
...
2. À l'init TLS, Babet sonde les chemins de CA bundles connus
